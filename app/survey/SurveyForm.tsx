...
/*
 * The form itself, split out from app/survey/page.tsx so the page can
 * stay a server component (it needs to read the signed-in user and, in
 * edit mode, their existing answers, before anything renders).
 *
 * The survey's two option lists are NOT plain `allGenres`/`allLengths`
 * — the export builds its own on top of them, and only here:
 *
 *   genreOpts  (`{{ isSurvey }}` block, Prototype with Admin.dc.html)
 *              = allGenres.concat(['Other'])
 *   lengthOpts (same block) = five prose labels mapped onto the four
 *              allLengths values plus a fifth, 'Any'
 *
 * `allGenres` itself stays unchanged — the search screen's genre
 * filters map it directly and must NOT gain an "Other" entry, which is
 * why both extras are built locally here rather than added to mock.ts.
 */

/* Display label -> the value stored on the survey, exactly as the
   export pairs them in the isSurvey block. */
...
export default function SurveyForm({
  ...
  const [level, setLevel] = useState(initial?.reading_level ?? allLevels[0]);
  const [length, setLength] = useState(initial?.preferred_length ?? allLengths[1]);
  ...
  // Exact copy from the export's isSurvey block: surveyKicker/surveyCta
  // switch on surveyEditing. Our onboarding kicker reads "STEP 4 OF 4",
  // not the export's "STEP 3 OF 4" — see the note in page.tsx on why
  // verify moved earlier in the flow.
  const kicker = editing ? "EDIT YOUR ANSWERS" : "STEP 4 OF 4";
  const cta = editing ? "Save and refresh recommendations" : "Find My Perfect Book!";
  const pendingLabel = editing ? "Saving…" : "Saving your answers…";
  ...
  return (
    <form action={formAction} style={{ maxWidth: 620, margin: "0 auto", padding: "48px 24px 72px" }}>
      ...
      {state?.error && (
        ...
        <div className="mono" style={{ color: "var(--color-problem-text)", marginBottom: 12 }}>
          {state.error}
        </div>
      )}
